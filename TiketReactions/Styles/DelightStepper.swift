...
@IBDesignable public class DelightStepper: UIControl {
    ...
    lazy var leftButton: UIButton = {
        ...
        button.addTarget(self, action: #selector(DelightStepper.leftButtonTouchDown), for: .touchDown)
        button.addTarget(self, action: #selector(DelightStepper.buttonTouchUp), for: .touchUpInside)
        button.addTarget(self, action: #selector(DelightStepper.buttonTouchUp), for: .touchUpOutside)
        button.addTarget(self, action: #selector(DelightStepper.buttonTouchUp), for: .touchCancel)
        return button
    }()
    ...
    lazy var rightButton: UIButton = {
        ...
        button.backgroundColor = self.buttonsBackgroundColor
        button.titleLabel?.font = self.buttonsFont
        button.addTarget(self, action: #selector(DelightStepper.rightButtonTouchDown), for: .touchDown)
        button.addTarget(self, action: #selector(DelightStepper.buttonTouchUp), for: .touchUpInside)
        button.addTarget(self, action: #selector(DelightStepper.buttonTouchUp), for: .touchUpOutside)
        button.addTarget(self, action: #selector(DelightStepper.buttonTouchUp), for: .touchCancel)
        return button
    ...
    lazy var label: UILabel = {
        ...
        let panRecognizer = UIPanGestureRecognizer(target: self, action: #selector(DelightStepper.handlePan))
    ...
    func setup() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(DelightStepper.reset), name: NSNotification.Name.UIApplicationWillResignActive, object: nil)
    }
...
extension DelightStepper {
    ...
    func scheduleTimer() {
        timer = Timer.scheduledTimer(timeInterval: timerInterval, target: self, selector: #selector(DelightStepper.handleTimerFire), userInfo: nil, repeats: true)
    }
...
/*
class DelightStepper: UIControl {
